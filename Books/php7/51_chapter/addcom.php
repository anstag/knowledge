...
try {
    ...
    // если ответ положительный, помещаем новое сообщение в базу данных
    ...
    if (!empty($_POST)) {
        ...
        // Если нет ошибок, помещаем сообщение в базу данных
        if (empty($error)) {
            $query = "INSERT INTO comments VALUES (NULL, :nickname, :content, NOW(), 1)";
    ...
    // 2. Выводим сообщения в порядке убывания даты из размещения
    $query = "SELECT * FROM comments WHERE is_visible = 1 ORDER BY created_at DESC";
    $com = $pdo->query($query);

    while ($comments = $com->fetch()) {
        // Обрабатываем сообщения перед выводом, чтобы исключить вставку JavaScript-кода
